"""
Hermes Business OS — Tareas Tool
...
class Tareas:
    ...
    def create_task(self, proyecto_id: str, titulo: str, descripcion: str = None,
                   responsable: str = None, fecha_vencimiento: str = None,
                   prioridad: str = "media", dependencias: List[str] = None,
                   etiquetas: List[str] = None) -> Dict[str, Any]:
        """
        Create a new task.
    ...
    def update_task(self, task_id: str, **updates) -> Dict[str, Any]:
        ...
        for task in self._tasks:
            if task["id"].lower() == task_id.lower():
                safe_updates = {k: v for k, v in updates.items() if k not in ["id", "proyecto_id", "fecha_creacion"]}
    ...
    def complete_task(self, task_id: str, comentario: str = None) -> Dict[str, Any]:
        ...
        result = self.update_task(task_id, estado="completada", fecha_completado=datetime.now().isoformat())
    ...
    def list_tasks(self, proyecto_id: str = None, estado: str = None,
                   responsable: str = None, prioridad: str = None) -> List[Dict[str, Any]]:
    ...
    def get_tasks_summary(self, proyecto_id: str = None) -> Dict[str, Any]:
        ...
        vencidas = len([t for t in tasks if t["estado"] in ["pendiente", "en_progreso", "bloqueada"] and datetime.fromisoformat(t["fecha_vencimiento"]) < datetime.now()])
    ...
    def _get_upcoming_deadlines(self, proyecto_id: str = None, days: int = 3) -> List[Dict[str, Any]]:
    ...
    def format_task_list(self, tasks: List[Dict[str, Any]], title: str = "Tareas") -> str:
        ...
        for task in tasks:
            status_emoji = status_emojis.get(task["estado"], "•")
            priority_emoji = priority_emojis.get(task["prioridad"], "")
            
            # Check if overdue
            fecha_vencimiento = datetime.fromisoformat(task["fecha_vencimiento"])
            is_overdue = fecha_vencimiento < datetime.now() and task["estado"] not in ["completada", "cancelada"]
            overdue_marker = " ⚠️ VENCIDA" if is_overdue else ""
            
            lines.append(f"{status_emoji} {priority_emoji} **{task['titulo']}** ({task['id']}){overdue_marker}")
            lines.append(f"   Estado: {task['estado']} | Prioridad: {task['prioridad']}")
            lines.append(f"   Responsable: {task['responsable']} | Vence: {fecha_vencimiento.strftime('%Y-%m-%d')}")
    ...
    def format_tasks_summary(self, summary: Dict[str, Any]) -> str:
        ...
        lines = [
            ...
            f"⚠️ Vencidas: {summary['vencidas']}",
            f"📈 Tasa de completado: {summary['tasa_completado']}%",
        ]
        
        if summary["proximas_vencimientos"]:
            lines.extend(["", "⏰ **Próximas vencimientos:**"])
            for item in summary["proximas_vencimientos"]:
                task = item["tarea"]
                emoji = "🔴" if item["dias_restantes"] <= 1 else "🟡"
                lines.append(f"{emoji} {task['titulo']} — {item['dias_restantes']} días ({item['fecha_vencimiento']})")
