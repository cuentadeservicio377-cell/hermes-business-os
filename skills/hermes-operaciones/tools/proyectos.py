"""
Hermes Business OS — Proyectos Tool
...
class Proyectos:
    ...
    def create_project(self, nombre: str, cliente_id: str = None, cliente_nombre: str = None,
                      descripcion: str = None, fecha_inicio: str = None,
                      fecha_entrega: str = None, monto: float = 0,
                      tipo: str = None, notas: str = None) -> Dict[str, Any]:
        """
        ...
        project = {
            ...
            "tipo": tipo or self.config.industria,
            "estado": "planificado",
            "monto": monto,
            ...
            "notas": notas or "",
            "historial": [
                {"accion": "Creación", "fecha": datetime.now().isoformat()}
            ],
        }
    ...
    def update_project(self, project_id: str, **updates) -> Dict[str, Any]:
        ...
        for project in self._projects:
            if project["id"].lower() == project_id.lower():
                # Don't allow updating id or historial directly
                safe_updates = {k: v for k, v in updates.items() if k not in ["id", "historial", "tareas", "checklist"]}
                project.update(safe_updates)
                project["historial"].append({
                    "accion": f"Actualización: {', '.join(safe_updates.keys())}",
    ...
    def update_status(self, project_id: str, nuevo_estado: str, notas: str = None) -> Dict[str, Any]:
        """Update project status."""
        if nuevo_estado not in self.PROJECT_STATUSES:
            return {
                "success": False,
                "error": f"Estado inválido. Válidos: {', '.join(self.PROJECT_STATUSES)}",
            }
    ...
    def list_projects(self, estado: str = None, cliente_id: str = None) -> List[Dict[str, Any]]:
    ...
    def get_projects_summary(self) -> Dict[str, Any]:
        ...
        avg_progress = sum(p["progreso"] for p in active_projects) / len(active_projects) if active_projects else 0
    ...
    def format_project_summary(self, summary: Dict[str, Any]) -> str:
        ...
        if summary["proximos_vencimientos"]:
            lines.extend(["", "⏰ **Próximos vencimientos:**"])
            for item in summary["proximos_vencimientos"]:
                proyecto = item["proyecto"]
                emoji = "🔴" if item["dias_restantes"] <= 2 else "🟡"
                lines.append(f"{emoji} {proyecto['nombre']} — {item['dias_restantes']} días ({item['fecha_entrega']})")
