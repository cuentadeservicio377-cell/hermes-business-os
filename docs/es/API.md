# Referencia API — Hermes Business OS Tools

Referencia de todas las herramientas Python disponibles en HBOS.
...
engine.run_full_onboarding()   # Dict con resultados
engine.get_onboarding_status() # Dict con estado
```
...
Todas las herramientas escriben a `data/*.json`. El directorio de datos se crea automáticamente en el primer uso.
...
Todas las herramientas siguen un patrón singleton:

```python
# Correcto — reusa instancia
crm = get_crm()

# También correcto pero crea nueva instancia
crm = CRM()
```

Usa las funciones `get_*()` en producción para asegurar consistencia de datos.
